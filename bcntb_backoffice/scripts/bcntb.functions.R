...
calculatePam50 <- function(exp.data.category.subset, PAM50.genes, column_name) {

  # loading required libraries
  suppressMessages(library('genefu'))

  # subsetting expression data on pam50 genes
  exp.data.category.subset.pam50 <- na.omit(exp.data.category.subset[PAM50.genes[,column_name],])
  # mapping ensembl ids to gene_symbols [necessary for genefu package]
  exp.data.category.subset.pam50 <- na.omit(merge(PAM50.genes[column_name],exp.data.category.subset.pam50,by="row.names",all.x = TRUE))[,-1]
