...
PAM50.genes <- read.table(file = opt$pam50, header = T, sep = "\t")
...
ann.data.splitted <- split(ann.data, ann.data$shortLetterCode)
...
for (j in 1:length(exp_files)) {
  ...
  if (length(as.character(ann.data.splitted$TP$File_name)) > 0) {
    selected_samples <- intersect(as.character(ann.data.splitted$TP$File_name),colnames(exp.data))
    exp.data.category.subset <- as.data.frame(t(scale(t(data.matrix(subset(exp.data[,-1], select = selected_samples))))))
    rownames(exp.data.category.subset) = exp.data$Gene.ID

    PAM50Preds <- calculatePam50(exp.data.category.subset, PAM50.genes, "Gene.Symbol")
