...
for (j in 1:length(cn_files)) {
    ...
    for(i in 1:length(unique(targets))) {
        ...
        layout( title = paste(target.sel,"(n=", length(target.sel),")", sep=""), yaxis = list(title = "Fraction of samples"), xaxis = list(title = "Chromosome number", showticklabels = FALSE), annotations = chr_nos, barmode = 'group', bargap = 0, margin = list(l=50, r=50, b=100, t=50, pad=4), autosize = F, legend = list(orientation = 'h', y = 1.07, tracegroupgap=0), showlegend=TRUE)
