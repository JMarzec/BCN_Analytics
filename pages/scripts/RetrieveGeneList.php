...
if ($type_analysis == "ccle") {
  $result_directory = "$absolute_root_dir/bcntb_backoffice/data/ccle/";
  chdir($result_directory);
  $expr_files = glob("gene_exp.csv");
} elseif ($type_analysis == "ccle_cnv") {
  $result_directory = "$absolute_root_dir/bcntb_backoffice/data/ccle/";
  chdir($result_directory);
  $expr_files = glob("cn_target_with_chr.csv");
} elseif ($type_analysis == "tcga") {
  $result_directory = "$absolute_root_dir/bcntb_backoffice/data/tcga/";
  chdir($result_directory);
  $expr_files = glob("gene_list.txt"); // in this case the "gene_exp.csv" file is too much big to manage
} else {
  $result_directory = "$absolute_root_dir/bcntb_backoffice/data/$ae"."_"."$pmid/norm_files/";
  chdir($result_directory);
